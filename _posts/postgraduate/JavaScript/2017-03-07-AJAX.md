...
`AJAX` stands for `A`synchronous `J`avaScript and `X`ML. 它是使用 [`XMLHttpRequest`](http://developer.mozilla.org/zh-CN/docs/Web/API/XMLHttpRequest)对象与服务器端脚本进行通信。它可以发送以及接收各种格式的信息，包括`JSON`，`XML`，`HTML`，甚至文本文件。
